...
Purpose of graph: These charts explore the difference between the number of subscribers and customers using CitiBike over the span of one year. 
...
Analysis: Considering that our timeframe highlights the peak of the pandemic, we suspect the dip from July 2019-April 2020 was due to a lockdown put into place from March to April 2020. This “dip” overtime is reflected in both user types. The gradual rise of users is reflective of New York’s four-phase reopening plan beginning in April 2020, which increased the number of users by July 2020. This visualization helps the CitiBike Marketing Team contextualize user patterns and adjust expectations on company profits from users due to travel restrictions during the pandemic. 
...
Analysis: We wanted to visualize the relative popularity of each CitiBike station in NYC. Working with the variable `start station`, we counted the number of trips starting from each station. Then we divided up the stations among 3 popularity categories (high, moderate, low). High popularity: count>5000; moderate popularity: 1000<count<5000; low popularity: count<1000. Based on these popularity groups, we created leaflet maps for each month in our dataset.
