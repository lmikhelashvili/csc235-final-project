...
leastpop_start<-mega_dataset_ivy%>%
...
```{r, message=FALSE, echo=FALSE, warning=FALSE}
mega_dataset_salwa <- mega_dataset%>%
  rename(start_station_name = 'start station name',
         start_station_id='start station id',
         start_station_latitude='start station latitude', 
         start_station_longitude='start station longitude', 
         ...
         end_station_name='end station name', 
         birth_year='birth year')
mega_dataset_salwa <- mega_dataset_salwa%>%
  mutate(starttime=as_datetime(starttime))%>%
  mutate(date=floor_date(starttime,unit="day"))
...
```{r, message=FALSE, echo=FALSE, warning=FALSE}
month_station_salwa<- mega_dataset_salwa%>%
  group_by(date,start_station_id,start_station_name,start_station_longitude,start_station_latitude)%>%
  summarise(count=n())%>%
  arrange(desc(count))
...
4:05
#subplot(plot1, plot2, plot3, plot4, plot5, plot6, nrows = 2, margin = 0.04, heights = c(0.6, 0.4))
#par(plot1, plot2, plot3, plot4, plot5, plot6, mfrow=c(2,3))
# p1 <- as.grob(ggarrange(plot1,ncol=2))
# p2 <- ggarrange(plot2, ncol=2)
# p3 <- ggarrange(plot3, ncol=2)
# p4 <- ggarrange(plot4, ncol=2)
# p5 <- ggarrange(plot5,ncol=2)
# p6 <- ggarrange(plot6, ncol=2, common.legend = TRUE, legend = "bottom")
#
# p1 <- annotate_figure(p1, top = text_grob("Summer 2019"))
# p2 <- annotate_figure(p2, top = text_grob("Winter 2019"))
# p3 <- annotate_figure(p3, top = text_grob("Winter 2019"))
# p4 <- annotate_figure(p4, top = text_grob("Winter 2019"))
# p5 <- annotate_figure(p5, top = text_grob("Winter 2019"))
# p6 <- annotate_figure(p6, top = text_grob("Winter 2019"))
#
# plot_grid(p1, p2, p3, p4, p5, p6, p7, p8, p9, ncol=3, labels=LETTERS[1:9])
#my_plot_list <- list(p1, p2, p3, p4, p5, p6)
# ggarrange(plotlist=my_plot_list, nrow=2, common.legend = TRUE, legend="bottom", heights = c(3,3.75))
# annotate_figure(p, top = text_grob("Main title", face = "bold", size = 16))
4:05

duration_dob<- mega_dataset_salwa %>%
  #mutate(starttime=as_datetime(starttime))%>%
  #mutate(stoptime=as_datetime(stoptime))%>%
  #mutate(align.time(hour, 5*60))+
  group_by(birth_year,tripduration)%>%
  summarise(count=n()>10)%>%
  arrange(desc(count))
# options(scipen=999)
# paste(format(round(count / 1e6, 1), trim = TRUE), "M")
# print.million <- function(x, quote = FALSE, ...) {
#     count <- paste0(round(x / 1e6, 1), "M")
...
#   return(labels)
# }
#Tried to make a graph of the durations for the mega_dataset but the dataset is too big
duration1<- ggplotly(
  april_2019_salwa%>%
    group_by(birth_year,tripduration)%>%
    summarise(count=n())%>%
